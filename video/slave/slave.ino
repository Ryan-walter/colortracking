...
void setup()
{
  ...
  bluetooth.println("F,1"); // This command causes the device to connect and immediately go into fast data mode using the stored remote address (master)
...
void right() // Function to turn right
{
  ...
  motors.setLeftSpeed(TURN_SPEED + 10);
  motors.setRightSpeed(-TURN_SPEED - 10);
  delay(600);
...
void keepTurning(int x) // Function meant to override parts of code so robot behaves differently when turning.
{
  ...
  if (res & LINE_VECTOR) // Robot will turn 90 degrees before it stops.
  {
    if (direc == 0) // Checks if previously turning left.
    {
      while (pixy.line.vectors->m_x1 < 39) // Continues to turn until head coordinate for the vector passes the halfway mark of Pixy's field of view
    ...
    {
      while (39 < pixy.line.vectors->m_x1) // Continues to turn until head coordinate for the vector passes the halfway mark of Pixy's field of view
      { // Signifies that robot fully turned 90 degrees.
        res = pixy.line.getMainFeatures();
